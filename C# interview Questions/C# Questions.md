...
2. The C# compiler converts it into **Intermediate Language (IL)** and stores it in an assembly (`.dll` or `.exe`).
3. The CLR loads the assembly and IL.
4. The **JIT compiler** converts IL into native machine code.
5. The CLR executes the code and provides runtime services such as garbage collection, exception handling, threading, and type safety.
